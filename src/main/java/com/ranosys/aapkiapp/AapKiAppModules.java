...
public class AapKiAppModules {

	public static WebDriver driver = null;
	public static List<ObjectStatus> status = new ArrayList<ObjectStatus>();
	public static int rowCount = 0;
	static String fileName = "E:\\SeleniumTestData\\AapKiAppStatus\\AapKiAppModulesStatus.xls";
	...
	@Test
	public static void executeAapKiAppModules() throws BiffException, IOException {
		Sheet s;
		FileInputStream fi = new FileInputStream("E:\\SeleniumTestData\\AapKiApp\\AapKiAppModules.xls");
		...
		for (int column = 4; column <= 4; column++) {
			for (int row = 1; row < rowNumbers; row++) {
				...
				try {
					...
					else if (TypofInput.equalsIgnoreCase("Text_Area1")) {
						Thread.sleep(5000);
						Robot rb = new Robot();
						rb.keyPress(KeyEvent.VK_TAB);
						rb.keyPress(KeyEvent.VK_TAB);
					...
					else if (TypofInput.equalsIgnoreCase("Reward_Date")) {
						((JavascriptExecutor) driver).executeScript("document.getElementById('LoyaltyReward_expiry_date').removeAttribute('readonly',0);");
						WebElement toDateBox = driver.findElement(By.id(Fieldname));
						toDateBox.clear();
						toDateBox.sendKeys(Value); // Enter date in required format
						toDateBox.sendKeys(Keys.RETURN);
					...
					else if (TypofInput.equalsIgnoreCase("Date_Car")) {
						((JavascriptExecutor) driver).executeScript("document.getElementById('Cars_registration_date').removeAttribute('readonly',0);");
						WebElement toDateBox = driver.findElement(By.id(Fieldname));
						toDateBox.clear();
						toDateBox.sendKeys(Value); // Enter date in required format
						toDateBox.sendKeys(Keys.RETURN);
					...
					else if (TypofInput.equalsIgnoreCase("Date_Holiday")) {
						((JavascriptExecutor) driver).executeScript("document.getElementById('Holiday_from_date').removeAttribute('readonly',0);");
						WebElement toDateBox = driver.findElement(By.id(Fieldname));
						toDateBox.clear();
						toDateBox.sendKeys(Value); // Enter date in required format
						toDateBox.sendKeys(Keys.RETURN);
